...
class Submit extends CI_Controller {
    ...
    public function index($problem_id = NULL) {
        $this->data = array(
            ...
            'single_problem' => $problem_id == NULL ? NULL : $this->assignment_model->problem_info($this->assignment['id'], $problem_id),
            'title' => 'Submit',
            'style' => 'main.css',
            'in_queue' => FALSE,
        ...
        if ($this->form_validation->run()) {
            $upload = $this->_upload();
            if ($upload !== FALSE)
                redirect('submit/status/' . $upload['submit_id'] . '/' . $upload['problem']);
        }
    ...
    private function _upload() {
        ...
        if ($this->user_model->get_user_level($this->username) == 0 && !$this->assignment['open'])
            show_error('Selected round has been closed.');
        if ($this->user_model->get_user_level($this->username) != 3 && $now < strtotime($this->assignment['start_time']))
            show_error('Selected round has not started.');
        if ($this->user_model->get_user_level($this->username) != 3 && $now > strtotime($this->assignment['finish_time']) + $this->assignment['extra_time'])
            show_error('Selected round has finished.');
        ...
        if ($_FILES['userfile']['error'] == 4) {
            ...
            if ($this->input->post('code') == "")
                show_error('No file chosen or code entered.');

            if (!file_exists($user_dir))
                mkdir($user_dir, 0700);

            //If no file is uploaded, check the code from the editor
            $EXT = filetype_to_extension($this->_language_to_type($this->input->post('language')));
            if ($EXT == "java") {
                $code = $this->input->post('code');
                $posOfClass = strpos($code, "class");
                $posOfClass += 5;
                $classname = "";
                while ($code[$posOfClass] !== '{' && $code[$posOfClass] !== '\n') {
                    $classname .= $code[$posOfClass++];
                }
                //echo $classname;
                $this->file_name = trim($classname);
            }
            else
                $this->file_name = 'submission';
            ...
            $submit_info = array(
                ...
                'file_name' => $this->file_name . '-' . $submit_id,
                'main_file_name' => $this->file_name,
                'file_type' => $this->_language_to_type($this->input->post('language')),
                'coefficient' => $this->coefficient
            );

            $flag = file_put_contents($user_dir . '/' . $submit_info['file_name'] . '.' . $EXT, $this->input->post('code'));
            echo $flag;
            if ($flag) {
            ...
            show_error("Couldn't write your code to file!");
        }
    ...
    public function status($submit_id = NULL, $problem_id = NULL) {
        $this->data = array(
            'username' => $this->username,
            'user_level' => $this->user_level,
            'all_assignments' => $this->assignment_model->all_assignments(),
        ...
        $this->load->view('templates/header', $this->data);
        $this->load->view('pages/submission_status', $this->data);
        $this->load->view('templates/footer');
    }
